...
import { useQuizMusic } from '../../components/useQuizMusic'

const store = useMainStore()
const router = useRouter()

const { isPlaying, play, stop } = useQuizMusic()
...
function currentTrack() {
  if (quizStep.value === 'quiz') return 'quiz'
  if (quizStep.value === 'result') return 'result'
  return 'lobby'
}
...
function goToStep(step) {
  quizStep.value = step
  if (!isMuted.value) {
    if (step === 'quiz') play('quiz')
    else if (step === 'result') play('result')
    else play('lobby')
  }
}
    ...
    <header class="sticky top-0 z-50 bg-emerald-900/95 backdrop-blur-md border-b border-emerald-700/40 shadow-lg">
      <div class="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          <button
            @click="toggleMusic"
            class="flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-full text-xs font-bold transition-all"
          >
            <span>{{ isPlaying ? '🔊' : '🔇' }}</span>
          ...
          <div v-if="isPlaying" class="flex items-end gap-0.5 h-4">
            <span
              v-for="i in 4" :key="i"
              class="inline-block w-1 bg-yellow-400 rounded-sm"
              :style="`animation: musicBar 0.6s ease-in-out ${i * 0.1}s infinite alternate; height: 6px`"
            ></span>
          </div>
      ...
      <transition name="fade" mode="out-in">
        ...
        <div v-if="quizStep === 'intro'" class="text-center animate-slideUp">
          ...
          <div v-if="questions.length > 0">
            <p class="text-emerald-800 font-medium mb-10 max-w-md mx-auto opacity-80">
              Kerjakan {{ questions.length }} soal evaluasi untuk mengukur pemahaman materi Pendidikan Agama Islam.
            </p>
            <div class="bg-white p-6 md:p-10 rounded-3xl shadow-xl border border-emerald-100 max-w-sm mx-auto">
              ...
              <input
                ...
                class="w-full p-4 border-2 border-slate-200 rounded-2xl focus:border-emerald-500 outline-none text-center font-bold text-lg text-emerald-900 mb-6 transition-all shadow-inner"
              >
              <button
                @click="startQuiz"
                class="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white font-black text-lg shadow-lg shadow-emerald-900/20 active:scale-95 transition-all"
              >
        ...
        <div v-else-if="quizStep === 'quiz'" class="bg-white rounded-3xl shadow-xl border border-slate-100 p-6 md:p-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <span
                  ...
                  :class="answers[idx] === opt ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400'"
                >{{ opt }}</span>
                <span class="font-bold text-slate-700 leading-snug">
          ...
          <div class="mt-14 pt-8 border-t border-slate-100">
            <button
              ...
              class="w-full py-5 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xl shadow-xl shadow-emerald-900/20 flex items-center justify-center gap-3 transition-all active:scale-95 disabled:opacity-50"
            >
        ...
        <div v-else-if="quizStep === 'result'" class="text-center animate-slideUp">
          <div class="bg-white rounded-3xl shadow-2xl p-10 md:p-16 border border-emerald-100 max-w-md mx-auto relative overflow-hidden">
            ...
            <div
              class="w-32 h-32 md:w-40 md:h-40 mx-auto mb-8 rounded-full border-[10px] flex items-center justify-center relative shadow-inner"
              :class="currentScore >= 70 ? 'border-emerald-500 text-emerald-600 bg-emerald-50' : 'border-red-400 text-red-500 bg-red-50'"
            >
            ...
            <p class="text-emerald-700/70 font-medium mb-10 leading-relaxed">
              Alhamdulillah, ujian selesai. Skor atas nama
              <strong class="text-emerald-900 border-b-2 border-yellow-300">{{ studentName }}</strong>
              sudah tercatat di sistem admin Guru.
            </p>

            <router-link
              to="/"
              class="block w-full py-4 rounded-xl border-2 border-emerald-600 text-emerald-700 font-black hover:bg-emerald-600 hover:text-white transition-all"
            >
              Kembali ke Beranda
            </router-link>
          </div>
